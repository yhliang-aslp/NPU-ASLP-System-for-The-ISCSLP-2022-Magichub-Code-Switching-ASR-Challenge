...
def get_args():
    ...
    parser.add_argument('--lm_weight',
                        type=float,
                        default=0.2)
    parser.add_argument('--lm_configs',
                        type=str,
                        default=None)
    parser.add_argument('--intern_lm_model_path',
                        type=str,
                        default=None)
    parser.add_argument('--intern_lm_weight',
                        type=float,
                        default=0.2)
    parser.add_argument('--intern_lm_configs',
                        type=str,
                        default=None)
...
def main():
    ...
    ## LM_init
    lm_configs = None
    if args.lm_configs != None:
        with open(args.lm_configs, 'r') as fin:
            lm_configs = yaml.load(fin, Loader=yaml.FullLoader)
    intern_lm_configs = None
    if args.intern_lm_configs != None:
        with open(args.intern_lm_configs, 'r') as fin:
            intern_lm_configs = yaml.load(fin, Loader=yaml.FullLoader)
    # Init asr model from configs
    model = init_asr_model(configs, lm_model_path=args.lm_model_path, lm_configs=lm_configs, intern_lm_model_path=args.intern_lm_model_path, intern_lm_configs=intern_lm_configs)
        ...
        for batch_idx, batch in enumerate(test_data_loader):
            ...
            # with other batch decoding mode
                ...
                hyp, _ = model.attention_rescoring(
                    ...
                    intern_lm_weight=args.intern_lm_weight,
                    simulate_streaming=args.simulate_streaming,
