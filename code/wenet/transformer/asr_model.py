...
from wenet.transformer.decoder import (TransformerDecoder,
                                       ...
                                       LTM_TransformerDecoder)
from wenet.transformer.encoder import ConformerEncoder
from wenet.transformer.encoder import TransformerEncoder
from wenet.transformer.encoder import BiEncoder, SharedEncoder
from wenet.transformer.transducer_model import *
...
class ASRModel(torch.nn.Module):
    ...
    def __init__(
        ...
        self.intern_lm_model = intern_lm_model
    ...
    def _ctc_prefix_beam_search(
        ...
        # cn_ctc_probs = self.ctc.log_softmax(
        #     cn_out)  # (1, maxlen, vocab_size)
        # en_ctc_probs = self.ctc.log_softmax(
        #     en_out)  # (1, maxlen, vocab_size)
        # ctc_probs += 0.3 * (cn_ctc_probs + en_ctc_probs)
    ...
    def attention_rescoring(
        ...
        #LM
        if self.lm_model != None:
            lm_out = self.lm_model.score(hyps_pad, hyps_lens, eos_flag=0, softmax_flag=0)
            lm_out = torch.nn.functional.log_softmax(lm_out, dim=-1)
            lm_out = lm_out.cpu().numpy()

        #intern_lm
        if self.intern_lm_model != None:
            intern_lm_out = self.intern_lm_model.score(hyps_pad, hyps_lens, eos_flag=0, softmax_flag=0)
            intern_lm_out = torch.nn.functional.log_softmax(intern_lm_out, dim=-1)
            intern_lm_out = intern_lm_out.cpu().numpy()
        ...
        for i, hyp in enumerate(hyps):
            ...
            if self.intern_lm_model != None:
                intern_lm_score = 0.0
                for j, w in enumerate(hyp[0]):
                    intern_lm_score += intern_lm_out[i][j][w]
                intern_lm_score += intern_lm_out[i][len(hyp[0])][self.eos]
                score += intern_lm_score * intern_lm_weight
            if score > best_score:
                best_score = score
                best_index = i
...
def init_asr_model(configs, lm_model_path=None, lm_configs=None, intern_lm_model_path=None, intern_lm_configs=None):
    ...
    elif encoder_type == 'biencoder':
        encoder = BiEncoder(input_dim,
                            global_cmvn=global_cmvn,
                            **configs['encoder_conf'])
    elif encoder_type == 'shared_encoder':
        encoder = SharedEncoder(input_dim,
                            global_cmvn=global_cmvn,
                            **configs['encoder_conf'])
    else:
    ...
    elif decoder_type == 'LTM_TransformerDecoder':
        decoder = LTM_TransformerDecoder(vocab_size, encoder.output_size(),
                                     **configs['decoder_conf'])
    else:
    ...
    if lm_model_path != None:
        lm_model = init_lm_model(lm_configs)
        model_state_dict = torch.load(lm_model_path, map_location=lambda storage, loc: storage)
        lm_model.load_state_dict(model_state_dict, strict=True)
        del model_state_dict
    else:
        lm_model = None

    if intern_lm_model_path != None:
        intern_lm_models = init_intern_lm_model(intern_lm_configs)
        model_state_dict = torch.load(intern_lm_model_path, map_location=lambda storage, loc: storage)
        intern_lm_models.load_state_dict(model_state_dict, strict=True)
        del model_state_dict
    else:
        intern_lm_models = None
    ...
    if decoder_type == 'transducer':
        ...
        model = ASRModel(
            ...
            ctc=ctc,
            lm_model=lm_model,
            intern_lm_model=intern_lm_models,
            **configs['model_conf'],
        )
    return model
