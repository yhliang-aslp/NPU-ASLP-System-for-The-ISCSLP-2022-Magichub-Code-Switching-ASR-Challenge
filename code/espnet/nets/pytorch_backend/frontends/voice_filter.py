...
class Frontend(nn.Module):
    ...
    def forward(self, x, ilens, e, max_norm, return_mask=False):
        """
        x: N x T x F
        e: N x D
        """
        ...
        # torchaudio.backend.sox_io_backend.save("/home/work_nfs5_ssd/yhliang/workspace/espnet_meeting/best.wav", wav.cpu(), 16000)
        # exit()
        wav = wav / th.max(th.abs(wav), dim=1)[0].unsqueeze(1) * max_norm
        # torchaudio.backend.sox_io_backend.save("/home/work_nfs5_ssd/yhliang/workspace/espnet_meeting/best.wav", wav.cpu(), 16000)
        # exit()

        return wav, ilens, None
