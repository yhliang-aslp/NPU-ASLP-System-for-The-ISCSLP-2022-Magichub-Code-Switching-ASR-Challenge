...
class SpeakerAttributeSpkDecoderFirstLayer(nn.Module):
    ...
    def forward(self, tgt, tgt_mask, asr_memory, spk_memory, memory_mask, cache=None):
        ...
        if self.concat_after:
            x_concat = torch.cat(
                (x, self.src_attn(x, asr_memory, spk_memory, memory_mask)), dim=-1
            )
            x = residual + self.concat_linear2(x_concat)
        else:
            x = residual + self.dropout(self.src_attn(x, asr_memory, spk_memory, memory_mask))
        if not self.normalize_before:
            x = self.norm1(x)
...
#         return x, tgt_mask, memory, memory_mask, to_spk
