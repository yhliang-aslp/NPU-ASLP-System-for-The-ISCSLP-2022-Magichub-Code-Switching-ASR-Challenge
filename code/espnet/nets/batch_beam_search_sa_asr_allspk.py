...
from espnet.nets.beam_search_sa_asr_allspk import BeamSearch
...
class BatchBeamSearch(BeamSearch):
    ...
    def score_full(
        self, hyp: BatchHypothesis, asr_enc: torch.Tensor, spk_enc: torch.Tensor,
        ...
        for k, d in self.full_scorers.items():
            ...
            if isinstance(d, AbsDecoder):
                scores[k], states[k] = d.batch_score(hyp.yseq, hyp.states[k], asr_enc, spk_enc)
            else:
                scores[k], states[k] = d.batch_score(hyp.yseq, hyp.states[k], asr_enc)
        return scores, states

    def score_partial(
        self, hyp: BatchHypothesis, ids: torch.Tensor, asr_enc: torch.Tensor, spk_enc: torch.Tensor,
    ) -> Tuple[Dict[str, torch.Tensor], Dict[str, Any]]:
        """Score new hypothesis by `self.full_scorers`.
        ...
        for k, d in self.part_scorers.items():
            if isinstance(d, AbsDecoder):
                scores[k], states[k] = d.batch_score_partial(hyp.yseq, ids, hyp.states[k], asr_enc, spk_enc)
            else:
    ...
    def search(self, running_hyps: BatchHypothesis, asr_enc: torch.Tensor, spk_enc: torch.Tensor) -> BatchHypothesis:
        ...
        scores, states = self.score_full(running_hyps, asr_enc.expand(n_batch, *asr_enc.shape), spk_enc.expand(n_batch, *spk_enc.shape))
        #import pdb;pdb.set_trace()
        ...
        # for others.
        part_scores, part_states = self.score_partial(running_hyps, part_ids, asr_enc, spk_enc)
