...
from espnet.nets.beam_search_sa_asr import BeamSearch
...
class BatchBeamSearch(BeamSearch):
    ...
    def score_full(
        self, hyp: BatchHypothesis, asr_enc: torch.Tensor, spk_enc: torch.Tensor, profile: torch.Tensor,
        ...
        for k, d in self.full_scorers.items():
            ...
            if isinstance(d, AbsDecoder):
                scores[k], states[k] = d.batch_score(hyp.yseq, hyp.states[k], asr_enc, spk_enc, profile)
            else:
                scores[k], states[k] = d.batch_score(hyp.yseq, hyp.states[k], asr_enc)
        return scores, states

    def score_partial(
        self, hyp: BatchHypothesis, ids: torch.Tensor, asr_enc: torch.Tensor, spk_enc: torch.Tensor, profile: torch.Tensor,
    ) -> Tuple[Dict[str, torch.Tensor], Dict[str, Any]]:
        """Score new hypothesis by `self.full_scorers`.
        ...
        for k, d in self.part_scorers.items():
            if isinstance(d, AbsDecoder):
                scores[k], states[k] = d.batch_score_partial(hyp.yseq, ids, hyp.states[k], asr_enc, spk_enc, profile)
            else:
                scores[k], states[k] = d.batch_score_partial(hyp.yseq, ids, hyp.states[k], asr_enc)
        return scores, states
    ...
    def search(self, running_hyps: BatchHypothesis, asr_enc: torch.Tensor, spk_enc: torch.Tensor, profile: torch.Tensor) -> BatchHypothesis:
        ...
        weighted_scores = torch.zeros(
            n_batch, self.n_vocab, dtype=asr_enc.dtype, device=asr_enc.device
        )
        scores, states = self.score_full(running_hyps, asr_enc.expand(n_batch, *asr_enc.shape), spk_enc.expand(n_batch, *spk_enc.shape), profile.expand(n_batch, *spk_enc.shape))
        #import pdb;pdb.set_trace()
        ...
        # for others.
        part_scores, part_states = self.score_partial(running_hyps, part_ids, asr_enc, spk_enc, profile)
        for k in self.part_scorers:
            weighted_scores += self.weights[k] * part_scores[k]
        # add previous hyp scores
        weighted_scores += running_hyps.score.to(
            dtype=asr_enc.dtype, device=asr_enc.device
        ).unsqueeze(1)
