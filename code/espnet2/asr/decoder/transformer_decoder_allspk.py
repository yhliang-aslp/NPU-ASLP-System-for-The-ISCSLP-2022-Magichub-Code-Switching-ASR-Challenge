...
from espnet.nets.pytorch_backend.transformer.attention import CosineDistanceAttentionAllspk
...
class BaseSAAsrTransformerDecoder(AbsDecoder, BatchScorerInterface):
    ...
    def __init__(
        ...
        self.cos_distance_att = CosineDistanceAttentionAllspk()
    ...
    def score(self, ys, state, asr_enc, spk_enc):
        """Score."""
        ys_mask = subsequent_mask(len(ys), device=ys.device).unsqueeze(0)
        logp, state = self.forward_one_step(
            ys.unsqueeze(0), ys_mask, asr_enc.unsqueeze(0), spk_enc.unsqueeze(0), cache=state
        )
        return logp.squeeze(0), state

    def batch_score(
        self, ys: torch.Tensor, states: List[Any], asr_enc: torch.Tensor, spk_enc: torch.Tensor,
    ) -> Tuple[torch.Tensor, List[Any]]:
        """Score new token batch.
        ...
        logp, states = self.forward_one_step(ys, ys_mask, asr_enc, spk_enc, cache=batch_state)    
        # transpose state of [layer, batch] into [batch, layer]
        state_list = [[states[i][b] for i in range(n_layers)] for b in range(n_batch)]
        return logp, state_list
