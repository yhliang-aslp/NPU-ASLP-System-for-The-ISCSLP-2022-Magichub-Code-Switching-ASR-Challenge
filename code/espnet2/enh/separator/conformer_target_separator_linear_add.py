...
from espnet.nets.pytorch_backend.conformer.encoder_speaker_add import (
...
class ConformerTargetSeparatorLinearAdd(AbsSeparator):
    def __init__(
        self,
        input_dim: int,
        num_spk: int = 1,
    ...
    def forward(
        ...
        if self.conformer is not None:
            ...
            f_mask = pad_mask
        masks = []
        xvectors = self.speaker_down(xvectors)
        for i in range(self._num_spk):
            xvec = xvectors[:, i]
            xvec = xvec.unsqueeze(1).repeat(1,input_length,1)
            y, _ = self.conformer_tgt.forward_xvec(x,f_mask,xvec)
            y = self.linear(y)
