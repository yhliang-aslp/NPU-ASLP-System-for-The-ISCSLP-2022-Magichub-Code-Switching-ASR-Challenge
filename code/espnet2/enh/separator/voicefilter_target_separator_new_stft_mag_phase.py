...
class VoiceFilter_new_stft_mag_phase(AbsSeparator):
    def __init__(
        self,
        input_dim: int= 257,
    ...
    def forward(
        ...
        phase = torch.atan2(input.imag+EPSILON, input.real+EPSILON)
        r_spec = torch.transpose(mags, 1, 2)[:, 1:]
        i_spec = torch.transpose(phase, 1, 2)[:, 1:]
        emb = xvectors
        ...
        masked = [ComplexTensor(torch.transpose(r_out_spec, 1, 2)*torch.cos(torch.transpose(i_out_spec, 1, 2)),torch.transpose(r_out_spec, 1, 2)*torch.sin(torch.transpose(i_out_spec, 1, 2)))]
