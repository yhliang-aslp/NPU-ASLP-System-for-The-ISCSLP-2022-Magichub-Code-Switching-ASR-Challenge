...
class ConformerTargetSeparator(AbsSeparator):
    def __init__(
        self,
        input_dim: int,
        num_spk: int = 1,
    ...
    def forward(
        ...
        for i in range(self._num_spk):
            xvec = xvectors[:, i]
            xvec = xvec.unsqueeze(1).repeat(1,((input_length-1)//2-1)//2,1)
            y, _ = self.conformer_tgt(torch.cat([x,xvec],axis=-1), f_mask)
            if self._input_layer == 'linear':
                y = self.linear(y)
                mask = self.nonlinear(y)
            elif self._input_layer == 'conv2d':
                y = y.transpose(1,2)
                y = self.transpose_1(y, output_size=[(input_length-1)//2])
                y = self.nonlinear(y)
                y = self.transpose_2(y, output_size=[input_length])
                y = self.nonlinear(y)
                mask = y.transpose(1,2)
