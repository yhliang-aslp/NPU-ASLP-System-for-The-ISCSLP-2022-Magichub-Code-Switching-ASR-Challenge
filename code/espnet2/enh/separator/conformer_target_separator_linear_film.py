...
from espnet.nets.pytorch_backend.conformer.encoder_speaker_film import (
...
class ConformerTargetSeparatorLinearFILM(AbsSeparator):
    def __init__(
        self,
        input_dim: int,
        num_spk: int = 1,
        ...
        if layers_total-layers_tgt >0 :
            ...
            conformer_outsize = adim
        else:
            self.conformer = None
            self.y_down = torch.nn.Linear(input_dim,adim)
        ...
        #self.speaker_down = torch.nn.Linear(spk_embed_dim,adim)
        self.r = torch.nn.Linear(spk_embed_dim,adim)
        self.h = torch.nn.Linear(spk_embed_dim,adim)
    def forward(
        self,
        ...
        for i in range(self._num_spk):
            ...
            xvec_r = self.r(xvec)
            xvec_h = self.h(xvec)
            y, _ = self.conformer_tgt.forward_xvec(x,f_mask,xvec_r,xvec_h)
            y = self.linear(y)
