...
class VoiceFilter_new_stft(AbsSeparator):
    def __init__(
        self,
        input_dim: int= 257,
        ...
        rnn_units=256,
        dropout=0.0,
        K="5,1;5,1;5,1;5,1;5,1;5,1",
        S="2,1;2,1;2,1;2,1;2,1;2,1",
        C="1,16,32,64,128,256,256",
        P="2,2,2,2,2,2",
    ...
    def forward(
        ...
        mags = torch.clamp(input.real**2 + input.imag**2, EPSILON)**0.5
        phase = th.atan2(input.imag+EPSILON, input.real+EPSILON)
        r_spec = torch.transpose(mags, 1, 2)[:, 1:]
        emb = xvectors
        
        
        encoder_out = []
        # N x 2 x F x T
        out = torch.unsqueeze(r_spec, 1)
        ...
        r_mask = out.squeeze(1)


        r_mask = 10 * ((1 - th.exp(-0.1 * r_mask)) / (1 + th.exp(-0.1 * r_mask)))
        
        r_out_spec = r_mask * r_spec
        r_out_spec = tf.pad(r_out_spec, [0, 0,1, 0])
        r_mask = tf.pad(r_mask, [0, 0,1, 0])
        masked = [ComplexTensor(torch.transpose(r_out_spec, 1, 2)*torch.cos(phase),torch.transpose(r_out_spec, 1, 2)*torch.sin(phase))]
        assert len(r_mask.shape) == 3 
        masks= [torch.transpose(r_mask,1,2)]
