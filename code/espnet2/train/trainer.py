...
class Trainer:
    ...
    def run(
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            # if dp_model.encoder.after_norm.weight.requires_grad:
            #     for k, p in model.named_parameters():
            #         if k.startswith('encoder' + "."):
            #             p.requires_grad = False
            #         elif k.startswith('frontend' + "."):
            #             p.requires_grad = True
            #     logging.info('freeze encoder')
            # elif dp_model.frontend.frontend.lstm.weight_ih_l0.requires_grad:
            #     for k, p in model.named_parameters():
            #         if k.startswith('encoder' + "."):
            #             p.requires_grad = True
            #         elif k.startswith('frontend' + "."):
            #             p.requires_grad = False
            #     logging.info('freeze frontend')
    ...
    def train_one_epoch(
        ...
        for iiter, (_, batch) in enumerate(
            ...
            if iiter % log_interval == 0:
                ...
                if use_wandb:
                    reporter.wandb_log()
            # yhliang fixed for cuda out of memory
