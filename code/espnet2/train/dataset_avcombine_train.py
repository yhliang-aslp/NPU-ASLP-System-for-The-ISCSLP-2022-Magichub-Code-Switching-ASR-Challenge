    ...
    def __getitem__(self, uid: Union[str, int]) -> Tuple[str, Dict[str, np.ndarray]]:
        ...
        for name in data:
            ...
            if value.dtype.kind == "f":
                ...
                value = value.astype(self.float_dtype) / 255
                # crop
                c, t, h, w = value.shape
                delta_w = random.randint(0, w-88)
                delta_h = random.randint(0, h-88)
                value = value[:, :, delta_h:delta_h+88, delta_w:delta_w+88]
                # horizon flip
                if random.random() < 0.5:
                    value = np.flip(value, 3)
                #normalize
                value = (value - 0.412) / 0.165
