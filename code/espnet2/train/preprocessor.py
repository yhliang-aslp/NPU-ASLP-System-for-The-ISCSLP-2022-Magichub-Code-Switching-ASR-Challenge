...
class CommonPreprocessor(AbsPreprocessor):
    ...
    def __call__(
        ...
        if self.text_name in data and self.tokenizer is not None:
            ...
            if 'tsot_text' in data:
                tsot_text = data['tsot_text']
                tsot_text = self.text_cleaner(tsot_text)
                tsot_tokens = self.tokenizer.text2tokens(tsot_text)
                tsot_text_ints = self.token_id_converter.tokens2ids(tsot_tokens)
                data['tsot_text'] = np.array(tsot_text_ints, dtype=np.int64)
