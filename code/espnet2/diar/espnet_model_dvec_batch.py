...
class ESPnetDiarizationModel(AbsESPnetModel):
    ...
    def forward(
        ...
        # 1. Encoder
        #pdb.set_trace()
        encoder_out,feats_lengths = self.encode(speech, speech_lengths)
        pred = self.decoder(encoder_out,feats_lengths)
        #pdb.set_trace()
        ...
        if spk_labels.dim()==0:
            spk_labels = spk_labels.unsqueeze(0)
        if pred.dim() == 2 and spk_labels.dim()==1 and spk_labels!=None and pred.size(0)==spk_labels.size(0):
    ...
    def encode(
        ...
        feats_lengths = ((feats_lengths-1)//2-1)//2
        assert encoder_out.size(0) == speech.size(0), (
            encoder_out.size(),
            speech.size(0),
        )
        #pdb.set_trace()
        return encoder_out,feats_lengths
